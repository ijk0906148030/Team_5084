...
def evaluate(gt_dir, ts_dir, mode):
    ...
    if mode == 'multi_cam':
        ...
        log_message = mm.io.render_summary(summary, formatters=mh.formatters, namemap=mm.io.motchallenge_metric_names)
        logger.info(f'\n{log_message}')
        
        # 保存总得分和详细评估结果到文件中
        results_path = os.path.join(ts_dir, 'results.txt')
        with open(results_path, 'w') as file:
            file.write("Total Score: IDF1 " + str(summary.idf1.OVERALL) + " + MOTA " + str(summary.mota.OVERALL) + " = " + str(summary.idf1.OVERALL + summary.mota.OVERALL) + "\n")
        ...
        for gt_file, ts_file in zip(gt_files, ts_files):
            ...
            if os.path.splitext(gt_file)[0] == os.path.splitext(ts_file)[0]:
                ...
                # 保存得分和详细评估结果到文件中
                results_path = os.path.join(ts_dir, f'{name}_results.txt')
                log_message = mm.io.render_summary(summary, formatters=mh.formatters, namemap=mm.io.motchallenge_metric_names)
                with open(results_path, 'w') as file:
                    file.write(f"Score for {name}: IDF1 {summary.idf1.OVERALL}, MOTA {summary.mota.OVERALL}\n")
                    file.write(log_message)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Evaluate multiple object tracking results.')
    parser.add_argument('--gt_dir', type=str, default='AICUP_MOT15',help='Path to the ground truth directory')
    parser.add_argument('--ts_dir', type=str, default='detect_MOT15/final_public_private/e6e_test_0.65_box_200',help='Path to the tracking result directory')
