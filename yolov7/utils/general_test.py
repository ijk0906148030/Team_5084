...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # 将 det 中的边界框坐标从 img_shape 缩放到 im0_shape
        det[:, :4] = scale_coords((768, 1280), det[:, :4], (720,1280)).round()
        # 判定边界框是否在原始图像的边界内
        mask = (det[:, 0] > 0.5) & (det[:, 1] > 0.5) & (det[:, 2] < 1280-5) & (det[:, 3] < 720-5)
        # print(mask)
        mask |= (det[:, 4] >= 0.25)
        # print(mask)
        det = det[mask]
        # det[:, :4] = scale_coords((720,1280), det[:, :4], (768, 1280)).round()
        output[xi] = det   
