...
class ResNeXt(nn.Module):
    ...
    def __init__(self, last_stride, bn_norm, with_ibn, with_nl, block, layers, non_layers,
        ...
        # self.lstm = nn.LSTM(input_size=2048,  # ResNet50的最後一個卷積層的輸出特徵深度
        #                     hidden_size=2048,   # 設置 LSTM 的隱藏單元數量來匹配 EmbeddingHead 的輸入維度
        #                     num_layers=1,      # LSTM層數量，根據需要設置
        #                     batch_first=True)  # 表示輸入和輸出的第一維為 batch_size
        self.random_init()
    ...
    def forward(self, x):
        ...
        # # print(x.shape)
        # # # 即使用全局平均池化将特征图的 spatial dimension 转换成 single feature vectors
        # lstm_input = F.adaptive_avg_pool2d(x, (1, 1))  # shape: [batch, channels, 1, 1]
        # # print(lstm_input.shape)
        # lstm_input = lstm_input.view(lstm_input.size(0), -1)
        # # print("x:",lstm_input.shape)
        # lstm_input = lstm_input.view(batch_size, -1)  # shape: [batch, channels]
        # # print("lstm_input:",lstm_input.shape)
        
        
        # # lstm_input = lstm_input.unsqueeze(1)  # add a time dimension, shape: [1, batch, channels]
        # # # LSTM 前向传播
        # lstm_output, (h_n, c_n) = self.lstm(lstm_input.unsqueeze(1))
        # lstm_output = lstm_output.view(batch_size, 2048, 1, 1)
        # # print("lstm_output:",lstm_output.shape)
        # # 获取最后时间点的输出
        # # lstm_output = lstm_output.squeeze(0)  # 移除时间维度，为 FC 层准备

        # # 接下来，您可以添加一个全连接层或其他操作来处理 LSTM 的输出
